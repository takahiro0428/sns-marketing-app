...
2. 「**承認済みドメイン**」セクションで以下が含まれていることを確認：
   ...
   - `sns-marketing-web.web.app`（Hosting サブサイトのドメイン）
...
本プロジェクトはサブサイト `sns-marketing-web` にデプロイされます。Web アプリ追加時（手順2.3）に「Firebase Hosting も設定する」にチェックを入れた場合、`sns-marketing-web` サイトが自動的に作成されます。

### 6.1 サイトの確認

1. Firebase Console の左メニューから「**Hosting**」をクリック
2. サイト一覧に `sns-marketing-web` が存在することを確認
   - 公開 URL: `https://sns-marketing-web.web.app`
3. サイトが存在しない場合は「**サイトを追加**」から `sns-marketing-web` を作成

### 6.2 カスタムドメイン（任意）

カスタムドメインを使用する場合：
1. Firebase Console の「**Hosting**」を開く
2. `sns-marketing-web` サイトを選択
3. 「**カスタムドメインを追加**」をクリック
4. ドメインを入力し、DNS 設定の指示に従う
...
5. **Callback URI**: `https://sns-marketing-web.web.app/callback`（カスタムドメイン使用時は置換）
6. **Website URL**: `https://sns-marketing-web.web.app`
7. 「**Save**」をクリック
...
4. **ターゲット**を設定：
   - **ターゲットタイプ**: `HTTP`
   - **URL**: `https://sns-marketing-web.web.app/api/scheduler/process`
   - **HTTP メソッド**: `POST`
...
### 基本動作
- [ ] `https://sns-marketing-web.web.app` にアクセスできる
- [ ] ログインページが表示される
- [ ] メール/パスワードで新規登録ができる
- [ ] Google ログインができる
- [ ] ログアウトができる
